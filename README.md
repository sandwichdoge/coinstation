...
aggregation, AI-driven analysis, and a **date-aware backtester**.
...
- 🤖 **AI analysis** — combines the technical snapshot with news (as of a chosen moment) into an `action` + 0–100 `confidence` + rationale. Uses OpenAI when a key is set; otherwise a transparent rule-based fallback.
- ⏪ **Backtesting** with a real *date mechanism* — every data path (prices, indicators, news, AI) is anchored to an `as_of` timestamp, so you can replay a strategy "as if it were that date."
...
| Backend | **Python** + FastAPI | Requirement #2 — "ease of integration." Python has the richest ecosystem for *every* other requirement: market data, pandas TA math, the OpenAI SDK, `feedparser`, dataframe backtesting. |
...
| AI | OpenAI SDK + rule-based fallback | App stays fully functional without a key. |
...
```bash
# optional: enable OpenAI analysis
...
cp .env.example .env          # optional — add OPENAI_API_KEY here
...
| Var | Default | Notes |
|---|---|---|
| `OPENAI_API_KEY` | _(empty)_ | If unset → rule-based analysis. |
| `OPENAI_MODEL` | `gpt-4o-mini` | Any chat-completions model you have access to. |
| `OPENAI_BASE_URL` | _(empty)_ | Point at an OpenAI-compatible endpoint (Azure/proxy/local). |
...
Backtest strategies: `macd`, `rsi`, `ema_cross`, `ai`, `buy_hold`.
...
- **AI** — `analyze` accepts `as_of`; the `ai` backtest strategy calls the *same* function at each evaluation bar with news limited to that date.
...
│   │       ├── ai.py            # OpenAI + rule-based fallback
...
- **Stronger AI**: switch to OpenAI structured outputs / function calling in `services/ai.py`; the response is already validated/normalized.
